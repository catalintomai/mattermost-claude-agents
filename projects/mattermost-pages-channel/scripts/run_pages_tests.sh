...
if [ "$RUN_MODEL" = true ]; then
    ...
    # channels/utils tests (markdown utilities added for wiki/pages)
    run_test "Model: Utils tests (channels/utils)" \
        "go test -v -count=1 -timeout 300s ./channels/utils -run '^(TestLooksLikeMarkdown|TestMarkdownToTipTapJSON)$'"
fi
...
if [ "$RUN_CHANNEL_INDEPENDENCE" = true ]; then
    ...
    run_test "Chan-Indep: API (WikiLinks)" \
        "go test -v -count=1 -timeout 300s ./channels/api4 -run '^(TestLinkWikiToChannelAPI|TestGetWikiLinksForChannelAPI|TestGetWikiLinksByWikiAPI|TestUnlinkWikiFromChannelAPI|TestWikiEndpointsRequireAuth)$'"
...
if [ "$RUN_FRONTEND" = true ]; then
    ...
    run_test "Frontend: Wiki RHS" \
        "npm run test -- src/components/wiki_rhs/wiki_rhs.test.tsx src/components/wiki_rhs/wiki_new_comment_view.test.tsx src/components/wiki_rhs/all_wiki_threads.test.tsx src/components/wiki_rhs/wiki_page_thread_viewer.test.tsx src/components/wiki_rhs/wiki_reply_comment.test.tsx --silent"
    ...
    run_test "Frontend: Other Components" \
        "npm run test -- src/components/active_editors_indicator/active_editors_indicator.test.tsx src/components/inline_comment_context/inline_comment_context.test.tsx src/components/search_results/post_search_results_item.test.tsx src/components/post_markdown/system_message_helpers.test.tsx --silent"
    ...
    run_test "Frontend: Redux" \
        "npm run test -- src/packages/mattermost-redux/src/actions/active_editors.test.ts src/packages/mattermost-redux/src/actions/page_threads.test.ts src/packages/mattermost-redux/src/actions/wikis.test.ts src/packages/mattermost-redux/src/reducers/entities/pages.test.ts src/packages/mattermost-redux/src/reducers/entities/wikis.test.ts src/packages/mattermost-redux/src/reducers/entities/active_editors.test.ts src/packages/mattermost-redux/src/reducers/requests/wiki.test.ts src/packages/mattermost-redux/src/reducers/entities/threads/threadsInTeam.test.ts src/packages/mattermost-redux/src/utils/post_list.test.ts --silent"
    ...
    run_test "Frontend: Utils" \
        "npm run test -- src/utils/page_outline.test.ts src/utils/page_utils.test.ts src/utils/draft_autosave.test.ts src/utils/tiptap_to_markdown.test.ts src/utils/markdown_roundtrip.test.ts src/utils/markdown_full_roundtrip.test.ts src/utils/bookmark_utils.test.ts --silent"
...
if [ "$RUN_E2E" = true ]; then
    ...
    if curl -s http://localhost:8065/api/v4/system/ping > /dev/null 2>&1; then
        ...
        if should_run_category editor; then
            run_test "E2E: Editor & Content" "npm run test -- pages_editor pages_editor_resilience pages_formatting pages_callout pages_emoji pages_mentions pages_link_bubble pages_slash_commands pages_file_attachment pages_video_upload pages_external_image_paste pages_paste_markdown test_outline_minimal test_outline_navigation page_markdown_export page_markdown_roundtrip --project=chrome"
        fi

        # --- Collaboration & Real-time (batched) ---
        if should_run_category collaboration; then
            run_test "E2E: Collaboration & Real-time" "npm run test -- pages_active_editors pages_concurrent_editing pages_realtime_sync pages_realtime_hierarchy pages_wiki_realtime_creation pages_realtime_wiki_ops pages_realtime_page_moves pages_comments pages_threads page_inline_comment_rhs --project=chrome"
        fi
        ...
        if should_run_category ui; then
            run_test "E2E: UI & Display" "npm run test -- pages_author_avatar pages_browser_edge_cases pages_modal_reopen page_consolidation --project=chrome"
        fi
