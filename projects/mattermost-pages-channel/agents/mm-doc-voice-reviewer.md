...
- **Referring to a person, the request, or the requester** — an external-consumption doc states its content on its own terms, never as someone's ask or as a reply to a request. Phrases like "the request was", "as requested", "per the request", "the requester", "the ask was", "you asked", "we were asked to", or a named individual ("Joram wants …") → tag `mm:PERSON_REF`, MUST_FIX. Rewrite to state the fact or feature directly ("This page covers X", "The test of whether …"); name a requirement or constraint, not the person who raised it. Carve-out: a technical `AccessRequest` / HTTP-request parameter (e.g. `AccessEvaluation(request)`) is a different word, not this pattern. Also enforced mechanically by the build's `E:person-ref` gate.
- **Describing a section's format instead of its content** — a heading or sentence that announces the section's shape ("… in one paragraph each", "in a nutshell", "the following table shows") rather than its substance → tag `mm:FORMAT_META`, SHOULD_FIX. Drop the descriptor and name the content ("The two approaches", not "The two approaches in one paragraph each"). The narrowest cases ("in N paragraph(s) each", "in a nutshell") are also caught by the build's `E:format-meta` gate.
...
- **Unverified motive / intent asserted as fact about an external or other-team decision** — a confident causal claim about *why* an outside party (Mattermost the org, the Boards/Playbooks team, Atlassian, a competitor) made a decision, stated as certainty with no citation, when the motive is not directly verifiable (only the *outcome* is). E.g. "Mattermost integrated Boards **specifically to** stop duplicating posts/channels/search", "Boards was **deliberately** built as a plugin **in order to** …", "the duplication that **motivated** the integration", "Atlassian deprecated X **because** they wanted Y" → tag `mm:UNVERIFIED_MOTIVE`, MUST_FIX. We can observe what was *done* and its structural *effect*; we usually cannot read the deciders' intent. Rewrite to the verifiable effect, or hedge the motive, or cite a source: "integrated into the server **specifically to stop** maintaining the parallel stack" → "later integrated into the server, **which ended** that parallel stack (independent of the internal reasons)". Detection cues: a motive/intent verb (`to stop`, `to avoid`, `in order to`, `so as to`, `motivated`, `because they wanted`, `with the goal of`) or an intent adverb (`deliberately`, `specifically`, `precisely`, `intentionally`, `on purpose`) attached to an **external party's** action, with no `file:line` / doc / public-statement citation. **Also flag the purpose-pinned effect clause** (the subtle one the structural-effect carve-out used to let through): a result participle (`removing`, `eliminating`, `fixing`, `solving`, `ending`, `closing`) on an external party's action tied to the concern under discussion by a pointing qualifier (`exactly this`, `precisely this`, `this very`, `specifically this`) — e.g. "integrated into core, **removing exactly this duplication**" — or an external move enlisted as "the … **lesson**" / a precedent proving the decision was made for the reason being argued. Both smuggle a cause in through an effect clause. **Carve-outs:** (1) the *proposed design's own* rationale ("the design uses a `Pages` table to avoid a join") — that is this doc's own intent, legitimately asserted; (2) a motive that *is* cited (a linked blog post, RFC, or commit message); (3) the **neutral** structural-effect form — "later integrated into the server, which ended that parallel stack" is correct, but the carve-out does **not** extend to an effect clause that pins the result to the concern under discussion (`removing exactly this X`) or deploys the move as causal evidence (the `lesson`/precedent framing above), which read as motive.
- **Narrating the investigative tool / command used to obtain a result, inside a claim** — stating *how* a fact was found (which command, grep, search, or diff produced it) in the published prose, rather than asserting the fact itself. E.g. "`gh search` for `RegisterPermissions` **returns zero hits**", "`grep AccessControl …` **→ nothing**", "a **registry grep returns 0 hooks**", "`git diff … --shortstat` **shows** ~182k lines" → tag `mm:TOOL_NARRATION`, MUST_FIX. The reader needs the finding, not the keystroke that produced it. Rewrite to the fact: "`gh search` for `RegisterPermissions` returns zero hits in Boards and Playbooks" → "Neither Boards nor Playbooks registers permissions, roles, or schemes"; "No plugin API exposes the engine (`grep … → nothing`)" → "No plugin API exposes the engine in master". Detection cues: a tool/command token (`gh search`, `gh api`, `grep`, `rg`, `ag`, `git diff`, `git log`, `find`) or a search-result phrase (`returns zero hits`, `→ nothing`, `0 hits`, `grep returns`, `shows N files`) presented as the support for a claim in body prose. **Carve-outs:** (1) the **Sources read / methods / reproducibility appendix**, whose purpose is to record how each figure was measured — commands belong there; (2) naming a code **symbol, file, or `file:line`** as the anchor for a claim ("`AccessControlPolicyStore.Save` takes no transaction — `store.go:1252`") — that is anchoring, not tool-narration; (3) a command that is itself the *subject being designed* (e.g. a proposed `mmctl` subcommand), not the method that found a fact.
- **Absolute capability-denial about the plugin/platform that a master API or registry hook contradicts** — a sweeping "a plugin **cannot** X", "there is **no** API/hook for X", "host integration is **impossible**", "**cannot reach at all**", "X is **unavailable** to plugins" stated as a blanket fact, when master actually exposes a *partial* API or registry hook (so the true claim is narrower) → tag `mm:OVERSTATED_CAPABILITY_DENIAL`, MUST_FIX. This recurred three times on this doc set: "no registry hook for search" (master has `registerSearchComponents`), "no API to drive notification dispatch" (master has `SendPushNotification`), "host surfaces a plugin cannot reach **at all**" (master has `registerPostTypeComponent` / `registerRootComponent` / `registerFilePreviewComponent` / `registerWebSocketEventHandler` / …). Before asserting a denial, check the plugin API (`server/public/plugin/api.go`, `hooks.go`) and the webapp registry (`webapp/channels/src/plugins/registry.ts`); if a partial mechanism exists, **scope the claim to the exact gap** (what the partial API/hook does NOT do) and **name what it DOES**. Rewrite: "no registry hook for search" → "a plugin can register a search *action/suggestions* component (`registerSearchComponents`) but cannot render native page results"; "no notification API" → "a plugin can send push via `SendPushNotification` and post-backed mentions via `CreatePost`, but there is no unified preference-aware dispatch for non-post events". Detection cues: an absolute-denial token (`cannot … at all`, `no … API`, `no … hook`, `impossible`, `unavailable to (a )?plugin`, `plugins (simply )?can't`) on a host/plugin capability, with no scoping of what the plugin *can* do and no `register…` / `api.go` check. **Carve-outs:** (1) a denial already scoped + paired with the partial capability ("can register X but not Y") is correct; (2) a genuinely-absent mechanism verified against master (plugins truly cannot register roles/schemes/permissions) — still cite the check; (3) the server-side *not-API-shaped* claims (synthetic membership, cross-table transactions) where the denial is about a transaction/embedding boundary, not a missing call.
...
| `mm:UNVERIFIED_MOTIVE` | A confident motive/intent claim about *why* an external or other-team party made a decision, stated as certainty with no citation ("Mattermost integrated Boards **specifically to** stop duplicating …", "**deliberately** built as a plugin", "the duplication that **motivated** the integration"). Only the action and its structural effect are verifiable, not the intent. Rewrite to the effect, hedge, or cite. Carve-outs: the proposed design's *own* rationale; a cited motive; the *neutral* structural-effect form (Pass 4) — but NOT "removing exactly this X" or a "the … lesson" precedent, which pin a cause through an effect clause |
| `mm:TOOL_NARRATION` | Narrating the investigative tool/command that produced a result inside a claim ("`gh search` … returns zero hits", "`grep …` → nothing", "a registry grep returns 0 hooks", "`git diff --shortstat` shows N"), rather than asserting the finding. Rewrite to the fact. Carve-outs: the Sources/methods appendix (commands belong there); a `file:line`/symbol anchor; a command that is the designed subject (Pass 4) |
...
| `mm:OVERSTATED_CAPABILITY_DENIAL` | A blanket "a plugin cannot X / no API/hook for X / impossible / cannot reach at all" that a master API or registry hook contradicts (recurred: search→`registerSearchComponents`, notifications→`SendPushNotification`, host surfaces→`registerPostTypeComponent`/…). Check `plugin/api.go` + `plugins/registry.ts`; scope the claim to the exact gap and name what the plugin CAN do. Carve-outs: already-scoped denials ("can register X but not Y"); verified-absent mechanisms (cite the check); server-side transaction/embedding-boundary claims (Pass 4) |
