...
| Group | Agents | When |
|-------|--------|------|
| Cross-cutting | `simplicity-reviewer`, `code-slop-reviewer`, `comment-opacity-reviewer`, `comment-prose-reviewer`, `naming-consistency-reviewer`, `db-call-reviewer`, `efficiency-reviewer`, `type-duplication-reviewer`, `structural-health-reviewer`, `separation-of-concerns-reviewer`, `code-reviewer`, `architecture-tradeoff-reviewer` | Always (all projects) |
...
| Compatibility | `backwards-compatibility-reviewer`, `batch-operations-reviewer`, `null-safety-reviewer`, `deprecation-reviewer`, `mm-deprecation-reviewer`, `license-reviewer`, `file-structure-reviewer`, `config-migration-reviewer`, `type-design-reviewer`, `client-server-alignment-reviewer`, `schema-necessity-reviewer`, `launch-readiness-reviewer` | `model/` changes, API surface changes, new files/dirs, config changes. **`mm-deprecation-reviewer` is NOT removal-gated** — on MM projects run it on ANY `*.go`/`*.ts` diff: its main job is catching new code that CONSUMES an already-deprecated API, which no linter covers (MM excludes staticcheck `SA1019` repo-wide). Do not skip it for "no deprecations/removals in this diff" — that reasoning describes the other half of its scope. |
| Infrastructure | `ci-failure-reviewer`, `ci-gate-reviewer`, `ci-design-reviewer` | CI/CD file changes or `--ci` flag |
| Security | `xss-reviewer`, `validation-reviewer`, `permission-reviewer`, `owasp-agentic-auditor`, `accessibility-reviewer`, `security-auditor`, `counterexample-reviewer` | `--thorough`. **`counterexample-reviewer` is additionally NOT thorough-gated**: run it on ANY diff that adds or modifies a guard, gate, validator, license check, or migration adoption path, or whose comments state an invariant ("fixed", "frozen", "only X can", "never Y") — it attacks the code's own safety claims, which no other agent does. |
...
| Python | `py-async-reviewer`, `py-datetime-reviewer`, `py-sqlite-reviewer` — **all three currently DISABLED** (files parked in `~/.claude/agents-disabled/review/`; re-enable before routing) | Python (`.py`) changes |
...
| Playbooks migrations | `playbooks-migration-reviewer` — **currently DISABLED** (parked in `%%MM_ROOT_DIR%%/.claude/agents-disabled/mattermost/migration/`) | `server/sqlstore/migrations.go` or `server/plugin.go` changes in playbooks plugin |
...
| `comment-prose-reviewer` | [CODE] | HOW changed comments are phrased — roundabout constructions, informal/anthropomorphic register, self-evident function narration, declarations narrated by consumer behavior, godoc narrating implementation mechanics, significance announcements. Judges the comment TEXT with no callee context. Split out of `comment-reviewer` (which kept accuracy, rot, duplication, structure) because these tells need no implementation and were reliably skipped alongside checks that do | `(root)` |
| `comment-opacity-reviewer` | [CODE] | Changed code comments a reader of that FILE alone cannot decode — undecodable coinages, empty role metaphors ("the generic sink"), unresolved referents ("those grants", "both"), invented compound modifiers. Paraphrases every changed comment; starved at the file boundary (never opens callees/docs). Owns the opacity shape `comment-reviewer` structurally cannot see, since that agent must read callees to check accuracy | `(root)` |
...
| `counterexample-reviewer` | [CODE] | Adversarial invariant attack — harvests the diff's own safety claims, builds an entity x operation mutation matrix, hunts name-list classification over categories with constructed members, treats adoption paths as hostile input (`_shared/hostile-adoption-rule.md`). Domain-general (permissions, licensing, caching, HA, transactions, config, lifecycle). Findings must be traced counterexamples, never speculation. | `(root)` |
...
> **All agents in this tier are currently DISABLED** — files parked in `~/.claude/agents-disabled/review/`. Move them back into `~/.claude/agents/review/` before routing a Python diff to them.

| Agent | Phase | Purpose | Location |
|-------|-------|---------|----------|
| `py-async-reviewer` | [CODE] | asyncio patterns: blocking calls, fire-and-forget tasks, missing cleanup, concurrent state mutation | `review/` **DISABLED** |
| `py-datetime-reviewer` | [CODE] | Naive datetime bugs, `utcnow()` deprecation, `strptime` without tz, mixed naive/aware comparisons | `review/` **DISABLED** |
| `py-sqlite-reviewer` | [CODE] | Python `sqlite3` module: WAL mode, parameterized queries, context managers, unbatched IN clauses, busy_timeout | `review/` **DISABLED** |
